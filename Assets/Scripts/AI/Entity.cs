...
namespace Ting.AI
{
    public abstract class Entity
    {
        ...
        public Entity(int entityType, Vector2D pos, double r)
        {
            this.pos = pos;
     ...
     */
        public static bool Overlapped<T, conT> (T ob, conT conOb) where T: Entity where conT: List<T>
        {
            return Overlapped(ob, conOb, 40.0);
        }
        
        public static bool Overlapped<T, conT>(T ob, conT conOb, double MinDistBetweenObstacles) where T: Entity where conT: List<T>
        {
            var it = conOb.GetEnumerator();
            while (it.MoveNext())
            {
                T tmp = it.Current;
        ...
        public static void TagNeighbors<T, conT>(T entity, conT ContainerOfEntities, double radius) where T : Entity  where conT : List<T>
        {
            //iterate through all entities checking for range
            var it = ContainerOfEntities.GetEnumerator();
            while (it.MoveNext())
            {
                T curEntity = it.Current;
                //first clear any current tag
                curEntity.UnTag();

                Vector2D to = Vector2D.sub(curEntity.pos, entity.pos);
